resource "aws_codebuild_project" "ami" {
  ...
  build_timeout  = "45"
  queued_timeout = "30"
  service_role   = "${data.terraform_remote_state.common.codebuild_info["iam_role_arn"]}"
  tags           = "${merge(local.tags, map("Name", "alfresco-ami-packer"))}"
  count          = "${length(local.tasks_list)}"

  logs_config {
    cloudwatch_logs {
      group_name  = "${data.terraform_remote_state.common.codebuild_info["log_group"]}"
